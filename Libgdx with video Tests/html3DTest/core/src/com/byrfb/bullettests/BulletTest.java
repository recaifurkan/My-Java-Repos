...
import com.byrfb.bullettests.bullet3dcontacttests.Input;
import com.byrfb.camerasystem.CameraController;
import com.byrfb.camerasystem.GhostCamera;
import com.byrfb.camerasystem.WorldInputProcessor;
...
public class BulletTest implements ApplicationListener {
	...
	public void create () {
		...
		cam = new GhostCamera(67, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
		...
		WorldInputProcessor processor = new WorldInputProcessor(camController,viewport);

		Input input = new Input(this);
		camController = new CameraController(cam);
		InputMultiplexer multiplexer = new InputMultiplexer(input,processor);
		...
		constructors.put("ground", new GameObject.Constructor(model, "ground", new BoxShape(new Vector3(2.5f, 0.5f, 2.5f)), 0f));
		constructors.put("sphere", new GameObject.Constructor(model, "sphere", new SphereShape(0.5f), 1f));
		constructors.put("box", new GameObject.Constructor(model, "box", new BoxShape(new Vector3(0.5f, 0.5f, 0.5f)), 1f));
		constructors.put("cone", new GameObject.Constructor(model, "cone", new ConeShape(0.5f, 2f), 1f));
		constructors.put("capsule", new GameObject.Constructor(model, "capsule", new CapsuleShape(.5f, 1f), 1f));
		constructors.put("cylinder", new GameObject.Constructor(model, "cylinder", new CylinderShape(new Vector3(.5f, 1f, .5f)),
			1f));
		...
		GameObject object = ((GameObject.Constructor) constructors.get("ground")).construct();
//	
...
//		object.body.setActivationState(CollisionObject.DISABLE_DEACTIVATION);
	...
	public synchronized void spawn () {
		GameObject obj = ((GameObject.Constructor) constructors.values[1 + MathUtils.random(constructors.size - 2)]).construct();
...
//		getInstances().get(0).body.translate(new Vector3(0, MathUtils.sinDeg(angle) * 0.15f, 0f));
	...
	public void dispose () {
		...
		for (Object ctor : constructors.values())
			((GameObject.Constructor) ctor).dispose();
		constructors.clear();
